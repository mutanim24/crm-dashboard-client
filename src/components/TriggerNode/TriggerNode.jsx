...
const TriggerNode = ({ data, selected }) => {
  return (
    <div className={`px-6 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl shadow-lg border-2 ${
      selected ? 'border-yellow-400 ring-2 ring-yellow-400 ring-opacity-50' : 'border-blue-700'
      ...
      <button
        ...
        data-node-id={selected ? selected.id : ''}
        onClick={(e) => {
          e.stopPropagation();
        }}
      ...
      <Handle 
        type="source" 
        position="right" 
        className="w-4 h-4 bg-blue-300 border-2 border-white rounded-full"
        style={{ right: '-8px' }}
      />
    </div>
  );
